...
class App(ttk.Frame):
    def __init__(self, parent):
        ...
        #状态
        self.state="Hold Editer"        #Hold Divider Mirror Offset
        #Hold Editer
        self.hold_type=[
            "Linear",
            "InSine",
    ...
    def setup_widgets(self):
        ...
        #Transit
        self.transit = ttk.Frame(self.choice_bar)
        self.setup_transit(self.transit)
        self.choice_bar.add(self.transit,text="Transit")
...
######################################################################################################################
    ...
    def MIRROR(self):
        ...
        try:
            d_number=-1
            number=eval(self.mirror.number_text.get('0.0','end'))
...
######################################################################################################################
    ...
    def OFFSET(self):
        ...
        try:
            ...
            offset_num=int(eval(self.offset.offset_num_entry.get()))
...
################################################################################################
    #Transit 部分
    def setup_transit(self,frame):
        #位移
        frame.move_label = ttk.Label(frame,text="位移:")
        frame.move_label.grid(row=0,column=0,sticky="w")
        frame.move_text = ttk.Entry(frame,width=10)
        frame.move_text.grid(row=0,column=1,sticky='w')
        frame.move_text.insert(0,'0')
        #提交
        frame.commit = ttk.Button(frame,text="run",width=13,command=self.TRANSIT)
        frame.commit.grid(row=1,column=0,columnspan=2)
        
        #输入
        frame.input_text=Text(frame,state='normal',width=65,height=12)
        frame.input_text.insert('0.0',"Input.")
        frame.input_text.grid(row=0,column=2,padx=5,pady=5,rowspan=40)
        #输出
        frame.output_text=Text(frame,state='normal',width=65,height=12)
        frame.output_text.insert('0.0',"Output.")
        frame.output_text.grid(row=40,column=2,padx=5,pady=5,rowspan=2)    

    def TRANSIT(self):
        output=[]

        move = int(self.transit.move_text.get().strip()) 
        INPUT=self.transit.input_text.get('0.0','end')
        INPUT=INPUT.split("\n")
        for line in INPUT:
            #文件头
            if "#" in line:
        ...
        self.transit.output_text.delete('0.0','end')
        self.transit.output_text.insert('0.0','\n'.join(output))
    #Transit 部分
################################################################################################
    # #Hold Divider 部分
